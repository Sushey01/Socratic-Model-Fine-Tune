#!/usr/bin/env python3
"""Merge Qwen2.5-7B QLoRA adapters, convert GGUF, upload to HF_QWEN25_REPO."""
...
from train_qwen25 import DEFAULT_HUB, DEFAULT_MODEL, DEFAULT_OUTPUT

ROOT = Path(__file__).resolve().parent
MERGED = ROOT / "merged_model_qwen25"
...
def merge(adapter: Path, base: str) -> Path:
    print(f"Merging {adapter} onto {base} (needs a lot of RAM)...", flush=True)
    tok = AutoTokenizer.from_pretrained(
        str(adapter) if (adapter / "tokenizer_config.json").is_file() else base,
        trust_remote_code=True,
    )
    try:
        model = from_pretrained_qwen(
            base,
...
def convert_gguf(merged: Path) -> Path:
    ...
    for outtype, name in (
        ("q8_0", "socratic-qwen25-7b-q8_0.gguf"),
        ("f16", "socratic-qwen25-7b-f16.gguf"),
    ):
        dest = GGUF_DIR / name
    ...
    raise SystemExit(f"GGUF convert failed ({last_err}). {MERGED} is kept.")
...
def main() -> None:
    ...
    if not adapter.is_dir():
        raise SystemExit(f"No 7B adapters at {adapter}. Train first: python start.py --qwen25")
    base = os.environ.get("BASE_MODEL") or DEFAULT_MODEL
    repo = (os.environ.get("HF_QWEN25_REPO") or DEFAULT_HUB).strip()
