#!/usr/bin/env python3
"""Merge Qwen QLoRA adapters, convert GGUF, upload to HF_QWEN_REPO under gguf/.

Uses merged_model_qwen/ so Phi-3 merged_model/ is not overwritten.
"""
...
def convert_gguf(merged: Path) -> Path:
    ...
    for outtype, name in (("q8_0", "socratic-qwen3-q8_0.gguf"), ("f16", "socratic-qwen3-f16.gguf")):
        dest = GGUF_DIR / name
...
def main() -> None:
    ...
    if not adapter.is_dir():
        raise SystemExit(f"No Qwen adapters at {adapter}. Train first: python start.py --qwen")
    base = os.environ.get("BASE_MODEL") or DEFAULT_MODEL
    repo = (os.environ.get("HF_QWEN_REPO") or DEFAULT_HUB).strip()
